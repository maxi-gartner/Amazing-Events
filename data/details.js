...
const cards= document.querySelector("#card_details");
const all_data = data.eventos.map((info) => {
    return info;
});
const filterData = all_data.filter(item => item.name.replace(/\s/g,'') === params)
const filterDataMap = filterData.map((a) => {
    return a.name
})
console.log(filterDataMap)

const addCards = filterData => {
